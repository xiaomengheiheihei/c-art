...
export default {
  mounted () {
    if (this.type === 'transparent') {
      const transparentElemStyle = this.$el.querySelector('.c-page-head-transparent').style
      const titleElem = this.$el.querySelector('.c-page-head__title')
      const iconElems = this.$el.querySelectorAll('.c-btn-icon')
      const iconElemsStyles = []
      const textColor = this.textColor
      for (let i = 0; i < iconElems.length; i++) {
        iconElemsStyles.push(iconElems[i].style)
      }
      const borderRadiusElems = this.$el.querySelectorAll('.c-page-head-btn')
      const oldColors = []
